...
async function loadLoanPageData() {
  const [loans, violations] = await Promise.all([
    readerApi.loans(),
    readerApi.violations(),
  ]);

  return { loans, violations };
}
...
function LoanList({ loans }) {
  ...
  return <div className="space-y-4">{loans.map((loan) => <LoanCard key={loan.MaMT} loan={loan} />)}</div>;
}
