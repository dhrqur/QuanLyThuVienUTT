...
export default function CatalogPage() {
  ...
  const load = useCallback(async () => {
    try { setError(""); setCatalog(await readerApi.catalog({ keyword, page, pageSize: 12 })); }
    catch (requestError) { setError(errorMessage(requestError)); }
  }, [keyword, page]);
  useEffect(() => { const timer = setTimeout(() => void load(), 0); return () => clearTimeout(timer); }, [load]);

  function search(event) { event.preventDefault(); setPage(1); setKeyword(query.trim()); }
  return <ReaderLayout><div className="space-y-6">
    ...
    {!catalog && !error ? <LoadingState /> : error ? <ErrorState message={error} retry={load} /> : <CatalogContent catalog={catalog} page={page} setPage={setPage} />}
  </div></ReaderLayout>;
}
...
function BorrowCart({ clear, items, remove, update }) {
  ...
  async function submit() {
    ...
    setSubmitting(true);
    try {
      await readerApi.createRequest({ LoaiYeuCau: "MUON", ChiTiet: items.map(({ MaSach, SoLuong }) => ({ MaSach, SoLuong })) });
