...
export default function ProfilePage() {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      setData(dashboard);
      setForm(createProfileForm(dashboard.profile));
    } catch (requestError) {
  ...
  return <ReaderLayout>{!data && !error ? <LoadingState /> : error ? <ErrorState message={error} retry={loadProfile} /> : <ProfileContent data={data} form={form} setForm={setForm} />}</ReaderLayout>;
}
...
function ProfileContent({ data, form, setForm }) {
  ...
  async function saveProfile(event) {
    ...
    try {
      await readerApi.updateProfile(form);
      toast.success("Đã cập nhật thông tin cá nhân");
    } catch (error) {
      toast.error(errorMessage(error, "Không thể cập nhật thông tin."));
    } finally {
      setSaving(false);
    }
  }
